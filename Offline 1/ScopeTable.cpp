...
SymbolInfo* ScopeTable::LookUp(string name){
    ...
    if(symbolinfo[index]->getName() == name){
        cout << "Found in ScopeTable# " + getID() + " at position " + to_string(index) + ", " + to_string(position) << endl;
...
bool ScopeTable::Delete(string name){
    ...
    if(symbolinfo[index]->getName() == name){
        ...
        cout << "Deleted Entry " + to_string(index) + ", " + to_string(position) + " from current ScopeTable\n";
        return true;
    }
    temp1 = symbolinfo[index];
